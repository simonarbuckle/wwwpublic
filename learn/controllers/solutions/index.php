<!DOCTYPE html>
<html>
	<head>
		<?php include '../../../include/_head.php' ?>
		<link rel="stylesheet" href="/styles/samples.css" type="text/css"/>
		<script type="text/javascript" src="/js/top-scrolling.js"></script>
		<script src="../../include/movies/quicktime.js" language="javascript"> </script>
	</head>
	<body class="documentation sampleslist">
		<div id="top"><a href="#top"></a></div>
		<?php include '../../../include/_header.php' ?>
		<div class="content-wrapper">
			<section class="content">
				<div class="wrapper">
					<?php include '../../include/learnHeader.html' ?>						
					<?php include '../../include/solutions/controllers/controllerSolutions.html' ?>					
				</div>
			</section>
		</div>		
		<?php include '../../../include/_footer.html' ?>
	</body>
</html>